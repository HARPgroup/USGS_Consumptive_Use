...
###########################################Pushing DMR timeseries Data to VAHydro#################################################
  ...
  consumptive_use <-'C:\\Users\\nrf46657\\Desktop\\VAHydro Development\\GitHub\\USGS_Consumptive_Use'


  library('readr')
  adminreg <-read_tsv(paste(consumptive_use ,'Documentation\\ECHO_VAHydro Imports','adminreg.txt',sep='\\'))
  ...
  facility.dataframe <- getFeature(facility_inputs,token, site, feature)
  ...
  release.dataframe <- getFeature(release_inputs, token, site, feature)
  ...
  outfall.dataframe <- getFeature(outfall_inputs, token, site, feature)
  ...
  conveyance.dataframe <- getFeature(conveyance_inputs, token, site, feature)
