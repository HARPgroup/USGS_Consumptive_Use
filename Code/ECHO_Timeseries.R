...
################################################Compile Data######################################################################
        ...
        ECHO_timeseries<-data.frame(FacilityName=Facility_Name,OutfallID=VPDESID,Statistic=statistic,Measured_Effluent=dmr_value,Units=dmr_units,Permitted_Limit=eff_limit,
                               MP_Begin_Date=mp_begin,MP_End_Date=mp_end,Violation_Code=violation,Violation_Severity=violation_severity)
        ECHO_timeseries<-ECHO_timeseries[!(is.na(ECHO_timeseries$MP_End_Date)),] #remove if a monitoring period end date is missing 
